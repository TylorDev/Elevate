...
export const MediaTimeline = ({ variant = 'oscilloscope' }) => {
  const { audio, playback } = useElevateVizIntegration()
  const { audioContext, analyser, audioElement: mediaElement } = audio
  const {
    currentTrackId,
    isPlaying,
  ...
  useEffect(() => {
    ...
    const setupAudio = () => {
      ...
      try {
        ...
        frozenDataRef.current = new Uint8Array(analyserNode.frequencyBinCount)
  ...
  }, [analyser, audioContext, currentTrackId, duration, isSimpleVariant, mediaElement])
  ...
  return (
    <div
      id="Otimeline"
      className={`${styles.root} waveform waveform-${normalizedVariant}`}
